...
class TurtleBotController:
    def __init__(self):
        ...
        self.webos_server_url = 'http://192.168.43.24:3000/waypoint_reached'
    ...
    def move_to_waypoint(self, waypoint):
        ...
        # 각 웨이포인트의 좌표 설정
        waypoints = {
            1: (6.655, -3.3564, -0.0036503, 0.99),
            2: (6.8391, -3.5672, -0.70455, 0.70966),
            3: (6.847, -3.9809, -0.708628, 0.705582),
            4: (6.8391, -3.5672, -0.70455, 0.70966),
            5: (6.7411, -3.2518, 0.00061159, 1),
            6: (6.0237, -3.3821, 0.0014067,1)
        }
    ...
    def result_callback(self, result):
        ...
        if result.status.status == 3:  # 목표 도달 확인
            ...
            self.notify_webos_server(self.current_waypoint)
            # 다음 웨이포인트로 이동
            self.current_waypoint += 1
            if self.current_waypoint > 6:  # 마지막 웨이포인트에 도달하면 네비게이션 종료
                self.end_navigation()
            else:
    ...
    def notify_webos_server(self, waypoint):
        """웨이포인트 도달 정보를 WebOS 서버에 전송"""
        data = {'waypoint': waypoint, 'status': 'reached'}
        try:
            response = requests.post(self.webos_server_url, json=data)
            if response.status_code == 200:
                rospy.loginfo(f"Successfully notified WebOS server of reaching waypoint {waypoint}")
            else:
                rospy.logwarn(f"Failed to notify WebOS server: Status Code {response.status_code}")
        except requests.exceptions.RequestException as e:
            rospy.logwarn(f"Error notifying WebOS server: {e}")
